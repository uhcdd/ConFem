...
try:
    import CaeFemElemC
    imp.reload(CaeFemElemC)
    from CaeFemElemC import *
...
def AssignGlobalDof( NodeList, ElList, MatList, NoIndToCMInd):              # assign dof indices
    for El in ElList:
        El.ElemDimData( NodeList,NoIndToCMInd )
        El.Ini2( NodeList,NoIndToCMInd, MatList)                                         # initialization of data depending on Sequence of nodes in NodeList which has been determined by Cuthill McKee
    Index = 0
    ...
    for El in ElList:                                                       # loop over all elements to assign global dof index to element table if element shares this dof (which is not mandatory)
        for j in range(El.nNod):                                            # loop over nodes of element
            ...
            set2 = No.DofT                                              # set of dof types of global node - not the same as element node
    ...
    for El in ElList:
        for j in range(El.nNod):
            set1 = El.DofT[j]
            for k, k0 in enumerate(set1):
                if k0==7: Mask[El.DofI[j,k]]=0                          # mask for dof type gradient damage
#    Skyline = np.zeros((Index), dtype=np.int32)                         # former python dtype=int (64bit), modified to make this compatible with linux-c++-swig (c int with 32 bit) 
#    SDiag = np.zeros((Index), dtype=np.int32)
...
#@profile
def IntForces( N, MatList, ElList, Dt, VecC,VecU,VecD,VecS,VecT, VecI, MatK,MatG,KVecU,KVecL,Skyline,SDiag, CalcType, ff, NLGeom, SymS, Buckl, eqiter):
    ...
    for i, Elem in enumerate(ElList):
        if Elem.Active:
            ...
            if NLGeom and Elem.NLGeomI and Elem.RotG: 
                ...
                Elem.UpdateElemData()
#            Elem.NoDisp[:] = uvec[:]    
            Elem.Nodisp = uvec       # nodal displacement to be stored in the element class - Ahmad ?????????????????

            # SDA internal equilibrium integration point loop
            NWiter = 6
#            NWiter = 7
            for witer in range(NWiter):                                     # iteration for internal equilibrium of SDA; !!! iteration limiter  is currently trial and error
                ...
                #
                if Elem.Type in ["CPS4S","CPE4S","CPS3S","CPE3S","C3D8_SDA","CPE6S","CPS6S"]: fw, Kww, Kuw, Kwu = Elem.CrIni( eqiter, witer, xvec)
                # integration point loop 
                for j in range(Elem.nIntL):                                     # build element stiffness with integration loop
                    if Elem.ShellRCFlag and j>=Elem.nIntLi:                     # for reinforcement layers
                        ...
                        f = Elem.Geom[0,0]*SampleWeight[InT,nint-1,j]           # weighting factor
                    if CalcType==10:                                            # mass matrix -- extra contributions to continuum in case of ShellRCFlag 
                        ...
                        continue
                    if Elem.RegType ==1:                                        # regularization types ???
                    ...
                    if Elem.Type in ["CPS4S","CPE4S","CPS3S","CPE3S","C3D8_SDA","CPE6S","CPS6S"]:
                        Bw = Elem.FormBw( r, s, t )                            # Mw comes here into play
...
#                    if CalcType==1: continue                                    # nodal forces only -- will not work with SDA convergence initial convergence control
        
                    f_ = det*f*Elem.Geom[1][0]
                    if Elem.RegType==1: kmat = kmat + f_ *(np.dot(np.transpose(B), np.dot(C,B))+np.dot(np.transpose(BN), np.dot(CR,BN)))# element stiffness
                    else:               kmat = kmat + f_ * np.dot(np.transpose(B), np.dot(C,B)) # element stiffness
...
#
##                        print('XYZ',np.linalg.det(np.dot(np.transpose(Bw),Bw)))
#                        KWW__ = f_*np.dot(np.transpose(Bw),np.dot(C,Bw))
#                        print('XYZ',Elem.Label,j,'\n',np.array([KWW__[0,0:3],KWW__[1,0:3],KWW__[2,0:3]]),'\n',np.array([Kww[0,0:3],Kww[1,0:3],Kww[2,0:3]]))
...
#                if Elem.Label==101:
#                    print('YYY',Elem.Type,j,det,f,Elem.Geom[1][0],f_,'__', Elem.Geom[0,0],SampleWeight[InT,nint-1,j],'\n',kmat)
#                    exit()
        ##                #sys.stdout.write('%6i\n'%(i.Label))
        #                print("XXX",Elem.Label,j,det,Elem.Geom[1][0],f,'\n')
        #                for kk in range(kmat.shape[1]): sys.stdout.write('%10i'%(kk+1))
        #                sys.stdout.write('\n')
        #                for k in range(kmat.shape[0]):
        #                    sys.stdout.write('%2i'%(k+1))
        #                    for kk in range(kmat.shape[1]): sys.stdout.write('%10.2e'%(kmat[k,kk]))
        #                    sys.stdout.write('\n')
        #                exit()
        # cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
                ...
                if Elem.Type in ["CPS4S","CPE4S","CPS3S","CPE3S","C3D8_SDA","CPE6S","CPS6S"]:
                    Elem.CrFin( fw, Kww, Dt, ff )    
                    kmat = kmat - np.dot( Kuw, np.dot(Elem.Dd,Kwu) )             # modification of bulk stiffness matrix
#                                rL = rL + dot( Kuw, dot(i.Dd,i.fwd))            # uhc belongs here to strict theory (?) but practically disturbes convergence behavior
                    Elem.Kwu = Kwu
                ...
                else: break                                         # break witer loop in case if there is no SDA element
                # end of SDA internal equilibrium integration point loop
##
#            if Elem.Type in ["CPS4S","CPE4S","CPS3S","CPE3S","C3D8_SDA","CPS6S","CPE6S"]:
#                Kww_ = np.array([Kww[0,0:3],Kww[1,0:3],Kww[2,0:3]])
##                if abs(np.linalg.det(Kww_))<ZeroD:
##                    print('YYY',Elem.Label,np.linalg.det(Kww_),'\n',Kww_)
#                if Elem.Label==1845:
#                    print('ZZZ',Elem.Label,np.linalg.det(Kww_),'\n',Kww_)

            sysMass += sum(sum(kmat))
            if CalcType==0: continue                                        # next element in case of system check
     
    #        for k in range(MatK.shape[0]): 
    #            for kk in range(MatK.shape[1]): sys.stdout.write('%10.2f'%(MatK[k,kk]))
    #            sys.stdout.write('\n')
    #        if Elem.Label==70:
    #            sys.stdout.write('%6i\n'%(Elem.Label))
    #            sys.stdout.write('  ')
    ...
    #        sys.stdout.write('  ')
    #        if True: #Elem.Type == 'B23I':
    #            print('XXX',Elem.Label)
    #            print('XXX',Elem.Label, file=ff)
    #            for kk in range(kmat.shape[1]): sys.stdout.write('%12i'%(kk))
    #            for kk in range(kmat.shape[1]): ff.write('%12i'%(kk))
    #            sys.stdout.write('\n')
    #            ff.write('\n')
    #            for k in range(kmat.shape[0]):
    #                sys.stdout.write('%6i'%(k))
    #                ff.write('%6i'%(k))
    #                for kk in range(kmat.shape[1]): 
    #                    sys.stdout.write('%12.3e'%(kmat[k,kk]))
    #                    ff.write('%12.3e'%(kmat[k,kk]))
    #                sys.stdout.write('\n')
    #                ff.write('\n')
    
    #        if NLGeom: gmat = np.dot(np.dot(Elem.Trans.transpose(),gmat),Elem.Trans) # transform local to global geometric stiffness # was for T2D2, but does actually not work for T2D2
            # Fill system vectors (internal forces) and matrices (stiffness matrix)
    ###        if Elem.Label in (5,6,7):   ###########################################################################################################################
    ###            print 'B', Elem.Label, i, rvec #np.dot(np.transpose(BN), sigR)
            ndof0 = 0
...
def EvNodeEl( NodeList, ElList, NoLabToInd):                                # evaluate element indices which each node is part of required for cuthill-mckee
    for i, el in enumerate(ElList):
        ...
        for j in el.Inzi:
            no = NodeList[j]
            no.NodeEl += [i]
            if elType in ["CPS3","CPE3","CPS4","CPE4"]:
            ...
            elif elType in ["T3D2E","T3D3E"]:
                if   no.Type=="":     no.Type = "T3DE"
                elif no.Type=="T3DE": pass
                else: raise NameError("EvNodeEl: exit 2",elType, no.Type)
            elif elType in ["T2D2E","T2D3E"]:
...
def FinishEquilibIteration( MatList, ElList, NodeList,NoIndToCMInd, ff, NLGeom, LoFl):
    Flag = False
...
#            kL = Elem.Data.keys()
        ...
        if len(Elem.StateVar) > 0:
            ...
            if Mat.Update:
                ...
                if not Flag and Flag2: Flag = True  
            else:
    ...
    for Elem in ElList:                                                 # loop over all elements ???
        if len(Elem.StateVar) > 0:                                      # ???
            Mat = MatList[Elem.MatN] 
            if Mat.Updat2: 
                Mat.UpdateStat2Var(Elem, ff, Flag, LoFl)     # ???
    for Elem in ElList:
        if Elem.Type in["CPE4S","CPE3S","CPS4S","CPS3S", "C3D8_SDA","CPE6S","CPS6S"]:
...
def Eigen( ne, lim, N, NodeList, ElList, Step, Kmat, Mmat, NoLabToNoInd,NoIndToCMInd, PloF, ff ): # NoLabToInd, mapNoInd, PloF, ff ):
    ...
    from ConFemInOut import PostElem3D
    ...
    for bl in Step.BoundList:                                           # loop over all boundary conditions of step
        ...
        for j, jj in enumerate(NodeList[nI].DofT):                      # loop over all dofs of node
            if jj==bl.Dof:                                              # dof type equals prescribed dof type 
                Found = True
                break 
        if not Found: raise NameError ("ConFemBasics::Eigen:missing correspondence for dof type")
        k = NodeList[nI].GlobDofStart + j                               # constrained dof global index
...
#        print('XXX',iiter,'\n',KK_,'\n',MM_)
    ...
    for i in range(ne):                                   
        ...
        if PloF: PostElem3D( ElList, NodeList,NoIndToCMInd, xx, 20., None)
