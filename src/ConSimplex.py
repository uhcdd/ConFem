...
#        if Elem.Rot: Angle = asin(Elem.Trans[0,1])*180/3.141593
#        else:        Angle = 0
...
class ConSimplex:
    ...
    def Run(self, Name, LinAlgFlag, PloF, ElPlotTimes=['1.0000']):
        ...
        f7=None
        print("ConSim: ", Name)
        NodeList, ElList, MatList, StepList, NoLabToNoInd, SecDic = ReadInputFile(f1, f6, False) # read input file and create node, element, material and step lists -> in ConFemInOut.py
        f1.close()
        ...
        if LinAlgFlag:
            ...
            IntForces( N, MatList,ElList, Time, VecZ,VecU,VecZ,VecZ,VecZ, VecI, None,None,KVecU,KVecL,Skyline,SDiag, 2, None,False,False,False, 0)# internal nodal forces / stiffness matrix
        else:
            MatK = sparse.lil_matrix((N, N))                    # sparse stiffness matrix initialization
            IntForces( N, MatList,ElList, Time, VecZ,VecU,VecZ,VecZ,VecZ, VecI, MatK,None,None,None,None,None,       2, None,False,False,False, 0)# internal nodal forces / stiffness matrix
        ...
        if LinAlgFlag:
            StepList[0].BoundCond( N, Time, 0, Time, NodeList,NoLabToNoInd,NoIndToCMInd, VecU, VecI, VecP, VecP0, BCIn, BCIi, None,KVecU,KVecL,Skyline,SDiag, 2, False)# introduce boundary conditions
        else:
            StepList[0].BoundCond( N, Time, 0, Time, NodeList,NoLabToNoInd,NoIndToCMInd, VecU, VecI, VecP, VecP0, BCIn, BCIi, MatK,[],None,None,None, 2, False)# introduce boundary conditions
        ...
        if LinAlgFlag:
            ...
            VecU[:] = VecR[:] #copy(VecR)
            IntForces( N, MatList,ElList, Time, VecZ,VecU,VecZ,VecZ,VecZ, VecI, None,None,KVecU,KVecL,Skyline,SDiag, 1, None,False,False,False, 0)# internal nodal forces
        else:
            ...
            IntForces( N, MatList,ElList, Time, VecZ,VecU,VecZ,VecZ,VecZ, VecI, MatK,None,None,None,None,None,       1, None, False,False,False, 0)# internal nodal forces
        WriteElemData( f2, f7, Time, ElList, NodeList,NoLabToNoInd,NoIndToCMInd, MatList, [])          # write element data
        ...
        f2.seek(0)
        PostElem2D( ElList, NodeList,NoIndToCMInd, MatList, f2, Sc, 1.0 , 1.0, ElPlotTimes)
...
if __name__ == "__main__":
    Name="../DataExamples/E06/E4-02u03"                         # input data name
#    Name="../DataExamples/E4/E4-01"                         # input data name
