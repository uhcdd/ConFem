...
import ConLinAlg
imp.reload(ConLinAlg)
from ConLinAlg import *

def Reinforcement2D( ElList, NodeList,NoIndToCMInd, ReinfDes, VecScales, KeyElset,KeyTime,KeyMat, ff, PlotFlag, Type):
...
#        print('XXX',degrees(phi1),'__',vphi1,vphi2,'__',vx,vy) for control purposes
    ...
    scaleP = 1.5*VecScales[KeyElset.strip()+KeyTime.strip()][0]
    if  Type.upper()=="PLATE":  
        ff.write('             x       y     sig_x   sig_y  sig_xy     phi_c  sig_c     rho_x  rho_y      as_x    as_y\n')
        PlotTitles = ["plate reinforcement","plate concrete struts","plate reinforcement ties"]
    elif Type.upper()=="SLAB": 
        ff.write('**                                  bottom                       top\n**     x       y     phicB    as_x    as_y     phicT    as_x    as_y     phi_1     v_1     v_2\n')
        PlotTitles = ["slab lower bending reinforcement","slab upper bending reinforcement","slab shear"]
...
#            i0 = FindIndexByLabel( NodeList, Elem.InzList[0])               # find node index from node label
#            i1 = FindIndexByLabel( NodeList, Elem.InzList[1])
#            i2 = FindIndexByLabel( NodeList, Elem.InzList[2])               # find node index from node label
#            i3 = FindIndexByLabel( NodeList, Elem.InzList[3])
#            n0, n1, n2, n3 = NodeList[i0], NodeList[i1], NodeList[i2], NodeList[i3]
#            xN = [n0.XCo,n1.XCo,n2.XCo,n3.XCo,n0.XCo]
#            yN = [n0.YCo,n1.YCo,n2.YCo,n3.YCo,n0.YCo]
            xN = [Elem.X0,Elem.X1,Elem.X2,Elem.X3,Elem.X0]
            yN = [Elem.Y0,Elem.Y1,Elem.Y2,Elem.Y3,Elem.Y0]
            p0.plot(xN,yN, 'b--')
            p1.plot(xN,yN, 'b--')
            p2.plot(xN,yN, 'b--')
#            xN = [n0.XCo,n1.XCo,n2.XCo,n3.XCo]                              # for usage with FormX
#            yN = [n0.YCo,n1.YCo,n2.YCo,n3.YCo]
...
#            i1 = FindIndexByLabel( NodeList, Elem.InzList[1])
#            i2 = FindIndexByLabel( NodeList, Elem.InzList[2])                 # find node index from node label
#            n0, n1, n2 = NodeList[i0], NodeList[i1], NodeList[i2]
#            xN = [n0.XCo,n1.XCo,n2.XCo,n0.XCo]
#            yN = [n0.YCo,n1.YCo,n2.YCo,n0.YCo]
            xN = [Elem.X0,Elem.X1,Elem.X2,Elem.X0]
            yN = [Elem.Y0,Elem.Y1,Elem.Y2,Elem.Y0]
            p0.plot(xN,yN, 'b--')
            p1.plot(xN,yN, 'b--')
            p2.plot(xN,yN, 'b--',linewidth=0.5)
#            xN = [n0.XCo,n1.XCo,n2.XCo]   # for usage with FormX
#            yN = [n0.YCo,n1.YCo,n2.YCo]
...
#           sigx, sigy, sigxy = Elem.Data[j,3], Elem.Data[j,4], Elem.Data[j,5]  # retrieve stresses/moments / loading
            ...
            if PlotFlag:
                ...
                if SS>=0: p0.plot([xI-SS*scaleP*pp[1],xI+SS*scaleP*pp[1]],[yI-SS*scaleP*pp[2],yI+SS*scaleP*pp[2]], Col_[Type.upper()+'pos'])
                else:     p0.plot([xI-SS*scaleP*pp[1],xI+SS*scaleP*pp[1]],[yI-SS*scaleP*pp[2],yI+SS*scaleP*pp[2]], Col_[Type.upper()+'neg'])
                SS = pp[3]
                if SS>=0: p0.plot([xI-SS*scaleP*pp[2],xI+SS*scaleP*pp[2]],[yI+SS*scaleP*pp[1],yI-SS*scaleP*pp[1]], Col_[Type.upper()+'pos'])
                else:     p0.plot([xI-SS*scaleP*pp[2],xI+SS*scaleP*pp[2]],[yI+SS*scaleP*pp[1],yI-SS*scaleP*pp[1]], Col_[Type.upper()+'neg'])
...
#                    print('XXX',Elem.Label,f"  {xI:.2f} {yI:.2f} __ {sigx:.2f} {sigy:.2f} {sigxy:.2f} __ {pp[0]:.2f} {pp[3]:.2f} __ {100*rhox:.2f} {100*rhoy:.2f} {degrees(phic):.1f} {sigc:.1f}")
                if PlotFlag and rhox>minRhoX: p0.text(xI,yI,format(100*rhox,".2f"),ha='center',va='bottom',color='blue',fontsize='x-large') # x-large, medium xx-large, small
                if PlotFlag and rhoy>minRhoY: p0.text(xI,yI,format(100*rhoy,".2f"),ha='center',va='top',color='blue', rotation=90,fontsize='x-large')
...
class ConPlaD:
    def __init__(self):
        pass
    def Run(self, Name, KeyElset,KeyTime,KeyMat, PloF, Type):
        print("ConPlaD: ", Name)
        stime = process_time()
        ...
        if pth.isfile(Name+".pkl"):                                         # read restart file if there is one
            ...
            NodeList=pickle.load(fd);ElList=pickle.load(fd);MatList=pickle.load(fd);StepList=pickle.load(fd);N=pickle.load(fd);WrNodes=pickle.load(fd);LineS=pickle.load(fd);Flag=pickle.load(fd);\
                ...
                NoLabToNoInd=pickle.load(fd);NoIndToCMInd=pickle.load(fd);
            fd.close()
        else: raise NameError ("ConPlaD: .pkl file missing")
        ...
        ASx, ASy, Vol = Reinforcement2D( ElList, NodeList,NoIndToCMInd, ReDes, Sc, KeyElset,KeyTime,KeyMat, f4, PloF, Type)
        f4.close()
...
#    numpy.seterr(all='raise')
    Name, KeyElset,KeyTime,KeyMat, PlotFlag, Type ="../DataExamples/E08/E6-02",' EL1', '1.0000', 'MAT1', True, "plate"                 # ConPlad Plate E4_01 linear elastic, reinforcement design        11s
    Name, KeyElset,KeyTime,KeyMat, PlotFlag, Type ="../DataExamples/E09/E7-01",'PROP1', '1.0000', 'MAT1', True, "slab"                         # input data name
    ConPlaD_ = ConPlaD()
    ConPlaD_.Run(Name, KeyElset,KeyTime,KeyMat, PlotFlag, Type)
