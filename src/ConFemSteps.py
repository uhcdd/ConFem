# ConFemSteps -- 2020-03-09
# Copyright (C) [2020] [Ulrich Haeussler-Combe]
...
#                nI = FindIndexByLabel( NodeList, i.NodeLabel)       # node index of bc
                ...
                i.ValOffset = VecU[k]
        return 0
    def BoundCond(self, N, Time, TimeS, TimeTar, NodeList,NoLabToNoInd,NoIndToCMInd, VecU, VecI, VecP, VecP0, BCIn, BCIi, Kmat,KVecU,KVecL,Skyline,SDiag, CalcType, SymS):# introduce loads and boundary conditions into system
        for i in self.BoundList:                                    # loop over all boundary conditions of step
            Found = False
            ...
            for j, jj in enumerate(no.DofT):                                    # loop over all dofs of node
                if jj==i.Dof:                                       # dof type equals prescribed dof type 
                    Found = True
                    break
            if not Found: raise NameError ("ConFemSteps.Step.BoundCond: missing correspondence for dof type. node",i.NodeLabel,no.DofT,"used by element?")
